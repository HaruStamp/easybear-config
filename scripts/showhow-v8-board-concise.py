...
def prompt_for(k):
    ...
    head = 'ภาพเดียว = เอกสารสตอรีบอร์ดดิจิทัลแนวตั้งเต็มเฟรม (ไม่ใช่ภาพถ่ายกระดาษ สมุด หรือหน้าจอ) ของคลิป "ทำให้ดู" เรื่อง "{item.productName}"'
    parts = [head]
    if k == 1:
        parts.append(block(('values.svSec>10', ' · ช่วงที่ 1 ของคลิป — ฉาก 1 ถึง 5')))
    else:
        parts.append(f' · ช่วงที่ {k} ของคลิป — ฉาก {first} ถึง {last}')
    parts.append(f'\nบังคับเลย์เอาต์: ตาราง 5 แถว × 4 คอลัมน์ มีแถวหัวคอลัมน์ เส้นแบ่งชัด · 1 แถว = 1 ฉาก เรียงบนลงล่าง ครบ {first}-{last} · ห้ามคอลลาจ/โปสเตอร์/แถบภาพที่ไม่มีตาราง'
                 '\nหัวคอลัมน์: ① ฉาก+เวลา ② ภาพ ③ มุมกล้อง ④ ')
    ...
    parts.append(' · ตัวหนังสือทุกช่องเป็นภาษาไทย ป้ายฉากคัดจากต้นบรรทัดของแต่ละแถวข้างล่างเป๊ะ ③④ ตัวเล็ก'
                 '\nภาพในช่อง ②: ภาพถ่ายสมจริงสไตล์สารคดีในบ้าน แสง {item.palette} · ทุกช่องสถานที่เดียวกัน ของที่งานไม่ได้แตะคงที่'
